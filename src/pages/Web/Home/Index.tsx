      ...
      <section className="mb-16 lg:mb-24">
        {loadingProfile ? (
          <div className="grid grid-cols-1 items-center gap-10 md:grid-cols-2 lg:gap-16">
            <div className="mx-auto h-80 w-full max-w-[420px] animate-pulse rounded-lg bg-gray-200 dark:bg-gray-900" />
            <div className="space-y-4">
              <div className="h-8 w-3/4 animate-pulse rounded bg-gray-200 dark:bg-gray-900" />
              <div className="h-4 w-full animate-pulse rounded bg-gray-200 dark:bg-gray-900" />
              <div className="h-4 w-5/6 animate-pulse rounded bg-gray-200 dark:bg-gray-900" />
              <div className="h-4 w-2/3 animate-pulse rounded bg-gray-200 dark:bg-gray-900" />
            </div>
          </div>
        ) : (
          profile && (
        ...
        <div className="flex flex-wrap justify-center gap-3 sm:gap-4 md:gap-5">
          {loadingCategories ? (
            Array.from({ length: 6 }).map((_, index) => (
              <span
                key={index}
                className="h-10 w-28 animate-pulse rounded bg-gray-200 dark:bg-gray-900"
              />
            ))
          ) : categories.length > 0 ? (
        ...
        {loadingPosts ? (
          <div className="space-y-4">
            {Array.from({ length: 3 }).map((_, index) => (
              <div
                key={index}
                className="h-28 animate-pulse rounded-lg bg-gray-100 dark:bg-gray-900"
              />
            ))}
          </div>
        ) : posts.length > 0 ? (
