...
import { HiChartPie, HiPencil, HiInbox, HiUser, HiArrowSmRight, HiTable } from "react-icons/hi";
...
export default function SidebarIndex(): JSX.Element {
  // Mendapatkan rute aktif
  const location = useLocation();
  const { pathname } = location;

  // Ambil user dari cookies, aman dari parsing error
  const user: User = (() => {
    try {
      ...
      <Sidebar aria-label="Admin Sidebar" className="h-full">
        <Sidebar.Items>
          <Sidebar.ItemGroup>
            ...
            {/* Content Management */}
            {hasAnyPermission(["categories.index", "posts.index", "pages.index"]) && (
              <Sidebar.Collapse label="Content Management" icon={HiPencil}>
                ...
                {hasAnyPermission(["posts.index"]) && (
                  <Sidebar.Item as={Link} to="/admin/posts" active={pathname === "/admin/posts"}>
                    Posts
                  </Sidebar.Item>
                )}
                {hasAnyPermission(["pages.index"]) && (
                  <Sidebar.Item as={Link} to="/admin/pages" active={pathname === "/admin/pages"}>
                    Pages
                  </Sidebar.Item>
                )}
              </Sidebar.Collapse>
            ...
            {hasAnyPermission(["photos.index", "sliders.index"]) && (
              <Sidebar.Collapse label="Media Management" icon={HiInbox}>
                {hasAnyPermission(["photos.index"]) && (
                  <Sidebar.Item as={Link} to="/admin/photos" active={pathname === "/admin/photos"}>
                    Photos
                  </Sidebar.Item>
                )}
            ...
            {/* Users Management */}
            {hasAnyPermission(["roles.index", "permissions.index", "users.index"]) && (
              <Sidebar.Collapse label="Users Management" icon={HiUser}>
                {hasAnyPermission(["roles.index"]) && (
                  <Sidebar.Item as={Link} to="/admin/roles" active={pathname === "/admin/roles"}>
                    Roles
                  </Sidebar.Item>
                )}
                ...
                {hasAnyPermission(["users.index"]) && (
                  <Sidebar.Item as={Link} to="/admin/users" active={pathname === "/admin/users"}>
                    Users
                  </Sidebar.Item>
                )}
              </Sidebar.Collapse>
